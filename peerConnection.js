...
const peerJsCallbacks = {
    onPeerOpen: (id) => {
        ...
        // window.peerJsMultiplayer should store the localId internally upon 'open'
        // The localId is accessible via getLocalId() later.
    ...
    onDataReceived: (data) => {
        ...
        if (data.type === 'full_state_update') {
            ...
            // Ensure game isn't reset by an old message after a new game started via reset_game
            if (state.gamePhase === state.GAME_PHASES.GAME_OVER && data.turnCounter > 0 && data.gameActive === true) {
                 // This check is trying to prevent an old game's state update from affecting a newly reset game.
                 // However, the turnCounter check below should generally handle this if TC is reset to 0 by init().
            }

            if (typeof data.turnCounter !== 'number' || (data.turnCounter <= state.turnCounter && state.turnCounter !== 0) ) { // Allow TC=0 if local TC is also 0 (fresh game)
            ...
            if (!state.gameActive) {
                ...
                } else { ui.updateStatus("Juego terminado."); }
                return;
            }
...
// Function to get the local PeerJS ID
...
// Initialize PeerJS session (can be called to ensure peer is connected to PeerServer and has an ID)
// This can be used by game.js before starting matchmaking.
export function ensurePeerInitialized(customCallbacks = {}) {
    if (window.peerJsMultiplayer?.init) {
        ...
        // For matchmaking, we mainly need onPeerOpen from customCallbacks.
        // Other callbacks (onConnectionOpen, onDataReceived etc.) should remain the main game ones.
        const effectiveCallbacks = {
            ...peerJsCallbacks, // Default game handlers
            onPeerOpen: (id) => { // Default onPeerOpen might be too specific for generic init
            ...
            // Retain other master callbacks like onNewConnection, onConnectionOpen, onDataReceived, onConnectionClose
            // from peerJsCallbacks if they are not meant to be overridden by this specific init call.
        };
...
export function initializePeerAsJoiner(hostIdFromUrl, stopPreviousGameCallback) {
    ...
    if(window.peerJsMultiplayer?.init) window.peerJsMultiplayer.init(null, peerJsCallbacks); // Uses the main peerJsCallbacks
...
export function connectToDiscoveredPeer(remotePeerId) {
    ...
    if (window.peerJsMultiplayer?.connect) {
        ...
        // Ensure that peerJsMultiplayer is already initialized with peerJsCallbacks
        // before this function is called. This should be handled by an earlier
        // call to ensurePeerInitialized or one of the host/joiner functions.
