...
export function hideOverlay(){
  document.getElementById('gameOverlay')?.style.setProperty('display','none');
  document.getElementById('statusOverlay')?.style.setProperty('display','none');
}
...
export function launchConfetti(){
  ...
  c.innerHTML = '';
  const count = 100,
        colors = ['#ff69b4','#ffc0cb','#ff1493','#ffe4e1','#db7093'];
...
export const highlightSuggestedMove        = i => {
  ...
  if(cells[i] && cells[i].querySelector('span')?.textContent==='') cells[i].classList.add('hint');
};
export const clearSuggestedMoveHighlight   = () => cells.forEach(c=>c.classList.remove('hint'));
...
export function updateScoreboard(){
  ...
  resultsDiv.innerHTML =
    `${me} <span id="myWinsSpan">${state.myWins}</span> – ` +
    `${opp} <span id="opponentWinsSpan">${state.opponentWins}</span> – ` +
    `🤝 <span id="drawsSpan">${state.draws}</span>`;
}

/* =========================================================================
   CENTRAL: keep all toggle buttons & switches in sync with state
   ========================================================================= */
  ...
  if(showStart && player1StartsBtn && randomStartsBtn && loserStartsBtn){ 
    [player1StartsBtn,randomStartsBtn,loserStartsBtn].forEach(b=>b.classList.remove('active'));
     const activeStartBtn = state.whoGoesFirstSetting === 'player1' ? player1StartsBtn :
                            ...
                            state.whoGoesFirstSetting === 'loser'   ? loserStartsBtn  : player1StartsBtn;
    activeStartBtn.classList.add('active');
  }

  if(cpuBtn){
    cpuBtn.disabled = false; 
    cpuBtn.classList.remove('disabled'); 
  }
...
export function hideQRCode(){
  if (qrDisplayArea) {
    qrDisplayArea.style.setProperty('display','none');
